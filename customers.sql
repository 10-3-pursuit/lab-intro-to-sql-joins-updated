...
-- --
SELECT orders.id as orders_id, customers.firstname, customers.email, orders.total FROM orders JOIN customers ON customers.id = orders.customerID WHERE orders.isPaid = TRUE;
-- --

\echo - Find all orders, including the firstname, lastname and email of the customer who made each order.
-- --
SELECT orders.id as order_id, customers.firstname, customers.lastname, customers.email FROM orders JOIN customers ON orders.customerID = customers.id;
...
-- --
SELECT customers.firstname, customers.email FROM customers  LEFT JOIN orders ON customers.id = orders.customerID WHERE orders.id IS NULL;
-- --


\echo - List the total spending of each customer along with their first name, last name and email.
-- --
SELECT SUM(total) as total_spending, customers.firstname, customers.lastname, customers.email FROM customers LEFT JOIN orders ON customers.id = orders.customerID GROUP BY customers.firstname, customers.lastname, customers.email;
-- --
...
-- --
SELECT customers.firstname, customers.lastname, COUNT(orders.id) as orders_made FROM customers LEFT JOIN  orders ON customers.id = orders.customerID GROUP BY customers.firstname, customers.lastname;
-- --

\echo - Find all customers who have spent more than 300 in total across all their orders.
-- --
SELECT customers.firstname, customers.lastname, SUM(total) as total_spending FROM customers LEFT JOIN orders ON customers.id = orders.customerID GROUP BY customers.firstname, customers.lastname HAVING SUM(total) > 300;
-- --


\echo - For each order, list the order total alongside the email of the customer, include only orders with totals above 400.
-- --
SELECT orders.id, customers.email, orders.total FROM orders LEFT JOIN customers ON customers.id = orders.customerID WHERE orders.total > 400;
-- --